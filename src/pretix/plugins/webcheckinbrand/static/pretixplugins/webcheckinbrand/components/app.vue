...
import { api } from '../api'
import type { CheckinList, Position, RedeemResponse, AnswerQuestion } from '../api'
...
async function fetchStatus () {
	...
	try {
		const data = await api.fetchStatus(checkinlist.value.id)
		logo.value = data.event?.logo ?? null
	} catch {
		// ignore
	}
}
...
const checkResultAmount = computed(() => checkResult.value?.total_checkins ?? '')
...
onMounted(async () => {
	...
	if (listHash) {
		const listId = listHash.substring(1)
		try {
...
<template lang="pug">
.container
	.header-logo(v-if="logo")
		img(:src="logo", alt="Brand Logo")

	Qrscanner(v-if="checkinlist", @qr-scanned="handleQRScan")
	...
	.panel.panel-primary.check-result(v-if="checkResult !== null")
		...
		.check-result-status(:class="'check-result-' + checkResultColor")
			...
			.check-result-item Aantal keer ingewisseld: {{ checkResultAmount }}
