...
export const api = {
	...
	async searchPositions (listId: number, query: string): Promise<PaginatedResponse<Position>> {
		const url = `${urls.lists}${listId}/positions/?ignore_status=true&expand=subevent&expand=item&expand=variation&check_rules=true&brand=true&search=${encodeURIComponent(query)}`
